...
function info() {
    // indicator info 
    return array(
        'title' => 'Number of children (16-17) provided with employment services',
        'headers' => array(
            'total'  => 'Total',
            'male'   => 'Male',
            'female' => 'Female'
...
function data($values = array()) {
    ...
    foreach ($monitoring_data as $data) {
        ...
        foreach ($individuals as $i_data) {
            ...
            $individual_age = $i_data['individuals/individual_age'];
            
            $unique = "$community_code-$household_id-$individual_id";
            if (!(($individual_age == 16) or ($individual_age == 17))) {
                unset($total[$unique]);
                unset($male[$unique]);
                unset($female[$unique]);
            } else if (array_key_exists($unique, $total) and !array_key_exists($unique, $status)) {
                $status[$unique] = true;
